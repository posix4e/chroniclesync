...
async function getSystemInfo(): Promise<DeviceInfo> {
...
async function syncHistory(forceFullSync = false): Promise<void> {
  try {
    ...
    const historyData = await Promise.all(historyItems.map(async (item) => {
      ...
      return visits
        .filter(visit => typeof visit.visitTime === 'number' && visit.visitTime >= startTime && visit.visitTime <= now)
        .map(visit => {
          ...
          return {
            url: item.url || '',
            title: item.title || '',
            visitTime: visit.visitTime || Date.now(), // Ensure we always have a number
...
async function loadSummaries(): Promise<void> {
  try {
    ...
    Object.values(storedSummaries).forEach((summary: any) => {
      if (summary && typeof summary === 'object' && 'url' in summary) {
        pageSummaries.set(summary.url, summary as PageSummary);
      }
    });
 ...
 */
async function processContentForSummarization(request: SummarizationRequest): Promise<void> {
  try {
...
chrome.runtime.onMessage.addListener((request: any, sender, sendResponse) => {
  if (request.type === 'getClientId') {
    ...
    // Process content for summarization
    processContentForSummarization(request as SummarizationRequest)
      .then(() => {
        sendResponse({ success: true });
      })
      .catch(error => {
