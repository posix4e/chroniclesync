...
const SAFARI_RESOURCES_DIR = join(ROOT_DIR, 'safari/ChronicleSync/ChronicleSync Extension/Resources');
...
async function main() {
  try {
    ...
    // Copy files to Safari extension resources
    console.log('Copying files to Safari extension...');
    
    // Ensure Safari resources directory exists
    await mkdir(SAFARI_RESOURCES_DIR, { recursive: true });
    
    // Copy all files to Safari resources
    for (const [src, dest] of filesToCopy) {
      await cp(
        join(ROOT_DIR, src),
        join(SAFARI_RESOURCES_DIR, dest),
        { recursive: true }
      ).catch(err => {
        console.warn(`Warning: Could not copy to Safari: ${src}: ${err.message}`);
      });
    }
    ...
    console.log('Extension packages created: chrome-extension.zip, firefox-extension.xpi, and Safari resources');
  } catch (error) {
