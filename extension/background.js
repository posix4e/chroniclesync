...
async function processContentForSummarization(request) {
  try {
    ...
    // Increase timeout for summarization process
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Summarization timed out')), 60000); // 60 seconds timeout
    });
    
    let summary;
    try {
      // Log the start of summarization
      console.log(`Starting summarization for: ${request.url}`);
      
      // Use a try-catch block for the summarization process
      summary = await Promise.race([
        summarizationService.processPage(pageContent).catch(error => {
      ...
      console.log(`Summarization completed successfully for: ${request.url}`);
    } catch (error) {
      console.warn(`Error in summarization process: ${error.message}. Using fallback summary.`);
      
      // Create a simple fallback summary using the first paragraph or sentences
      let fallbackSummary;
      try {
        // Try to extract first paragraph
        const firstParagraph = request.content.split('\n\n')[0] || request.content.substring(0, 200);
        
        // If paragraph is too short, try to extract first few sentences
        if (firstParagraph.length < 100) {
          const sentences = request.content.match(/[^.!?]+[.!?]+/g) || [];
          fallbackSummary = sentences.slice(0, 3).join(' ');
        } else {
          fallbackSummary = firstParagraph.length > 150 ? firstParagraph.substring(0, 150) + '...' : firstParagraph;
        }
      } catch (fallbackError) {
        // If all else fails, just use the first 150 characters
        console.error(`Error creating fallback summary: ${fallbackError.message}`);
        fallbackSummary = request.content.substring(0, 150) + '...';
      }
      ...
      summary = {
        ...
        isAutoGenerated: true // Flag to indicate this is a fallback summary
      };
      
      console.log(`Created fallback summary for: ${request.url}`);
    }
